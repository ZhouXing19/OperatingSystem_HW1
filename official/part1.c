...
{ 
	...
	int t = 1; //iterator of strings for "echo"
...
int parseSemiCol(char* str, char** semi_seped, char* two_D_semi_seped[MAXLIST][MAXLIST]){
	int i = 0;
    int j = 0;
    int k = 0;
    ...
    while(semi_seped[j]!=NULL){
        two_D_semi_seped[j][k] = strtok(semi_seped[j]," ");
        while(two_D_semi_seped[j][k]!=NULL){
            k++;
            two_D_semi_seped[j][k] = strtok(NULL," ");
        }
        j++;
    }
	//返回有多少个被分号分割的指令。
...
{ 
	...
	semicnt = parseSemiCol(str,semi_sep,multi_semi_col);
